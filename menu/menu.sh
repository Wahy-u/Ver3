#!/bin/bash
BURIQ () {
    curl -sS https://raw.githubusercontent.com/Wahy-u/ipvps/main/disini > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/Wahy-u/ipvps/main/disini | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/Wahy-u/ipvps/main/disini | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/Wahy-u/ipvps/main/disini | grep $MYIP | awk '{print $3}')
fi
...
clear
figlet 'Wahyu store' | lolcat
echo -e "${BICyan} ┌─────────────────────────────────────────────────────┐${NC}"
echo -e "${BICyan} │                  ${BIWhite}${UWhite}Server Informations${NC}"
echo -e "${BICyan} │"
echo -e " ${BICyan}│  ${BICyan}Use Core        :  ${BIPurple}WAHYU-TUNNELING${NC}"
...
echo -e "     ${BICyan}[${BIWhite}03${BICyan}] VLESS   ${BICyan}[${BIYellow}Menu${BICyan}]${NC}"  "     ${BICyan}[${BIWhite}08${BICyan}] RUNNING      ${NC}" " ${BICyan}     │"
...
echo -e "       ${BICyan}│  Expiry script${NC} : ${BIYellow}$exp2${NC} Days"
echo -e "       ${BICyan}└─────────────────────────────────────┘${NC}"
echo
read -p " selected menu [ 1 / 10 ] >>>>   " opt
echo -e ""
case $opt in
1) clear ; menu-ssh ;;
2) clear ; menu-vmess ;;
